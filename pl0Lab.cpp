...
void statement(symset fsys)
{
	...
	{ // variable assignment
	  //
		int id_index[MAX_CONTINUOUS_ASSIGN];
		int k = 0;
		//
		mask* mk;
		...
		while (sym == SYM_IDENTIFIER) {
			...
			else // 如果不是“:=”说明后面应当是expression。预测分析遇到冲突 
			{
				k--;//记录在id_index数组的标识符数量-1，
				conflict = 1;//预测分析遇到冲突 
				sym = previous_sym;//恢复之前接受的符号 
				expression(fsys);
				//printf("here\n");
				//error(13); // ':=' expected.
			}
		...
		for (int id_num = k; id_num >= 0; id_num--) {//通过登记标识符的数组id_index从右往左赋值
			mk = (mask*)&table[id_index[id_num]];
			if (id_index[id_num]) {
				if (id_num == k) {//表达式的值赋给标识符
...
*/
	...
	//2018
		...
		if (sym == SYM_ELSE) {
			cx2 = cx;	
			gen(JMP, 0, 0);
			code[cx1].a = cx;//出现了else再对code[cx1]修正
			getsym();
			statement(fsys);
			code[cx2].a = cx;
		}
...
int main()
{
	...
	statbegsys = createset(SYM_BEGIN, SYM_CALL, SYM_IF, SYM_WHILE, SYM_NULL, SYM_BREAK, SYM_CONTINUE, SYM_IDENTIFIER,SYM_ELSE);
